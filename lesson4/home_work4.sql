...
-- Подзапрос
SELECT s.shopname, name_cat.`name`
FROM shops s
JOIN (SELECT `name`, id
FROM cats  WHERE `name` = "Murzik") AS name_cat
ON s.id = name_cat.id; 
...
FROM shops s
JOIN (SELECT `name`, id
FROM cats  WHERE `name` != "Murzik" and `name` != "Zuza") AS name_cat
ON s.id = name_cat.id; 
...
SELECT o.ord_id, o.ord_datetime, a.an_name, a.an_price 
FROM analysis a
JOIN orders o 
ON a.an_id = o.ord_an 
WHERE DATE(o.ord_datetime) BETWEEN '2020-02-05' AND '2020-02-13'
ORDER BY o.ord_id;
